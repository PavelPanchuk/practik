...
class MainApp(App):
    ...
    def test(self):
        ...
        if ("Спорт" == valuetxt):
            key=new_d['Спорт']
            new_d.update(Спорт=(t + key))
        if ("Работа" == valuetxt):
            key=new_d['Работа']
            new_d.update(Работа=(t + key))
        if ("Сон" == valuetxt):
            key=new_d['Сон']
            new_d.update(Сон=(t + key))
        if ("Прогулка" == valuetxt):
            key=new_d['Прогулка']
            new_d.update(Прогулка=(t+ key))
        if ("Завтрак" == valuetxt):
            key=new_d['Завтрак']
            new_d.update(Завтрак=(t + key))
        if ("Обед" == valuetxt):
            key=new_d['Обед']
            new_d.update(Обед=(t + key))
        if ("Ужин" == valuetxt):
            key=new_d['Ужин']
            new_d.update(Ужин=(t+ key))
        if ("Развлечения" == valuetxt):
            key=new_d['Развлечения']
            new_d.update(Развлечения=(t + key))
        if ("Магазин" == valuetxt):
            key=new_d['Магазин']
            new_d.update(Магазин=(t + key))
        if ("Учеба" == valuetxt):
            key=new_d['Учеба']
            new_d.update(Учеба=(t + key))
        if ("Проезд" == valuetxt):
            key=new_d['Проезд']
            new_d.update(Проезд=(t+ key))
        if ("Игры" == valuetxt):
            key=new_d['Игры']
            new_d.update(Игры=(t + key))
        if ("Чтение" == valuetxt):
            key=new_d['Чтение']
            new_d.update(Чтение=(t + key))
        if ("Физра" == valuetxt):
            key=new_d['Физра']
            new_d.update(Физра=(t + key))
        if ("Другое" == valuetxt):
            key=new_d['Другое']
            new_d.update(Другое=(t + key))
        j = json.dumps(new_d)
        with open('my_data.json', 'w') as f:
            f.write(j)
